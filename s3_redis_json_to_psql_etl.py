...
def get_s3_jsons(sparksession, s3_full_path):
    ...
    try:
        a_d_f = sparksession.read.json(
            "s3a://" + s3_full_path, multiLine=True
        ).dropDuplicates()
        return a_d_f
...
def get_redis_jsons(redishost, redisport):
    ...
    adf = spark.read.json(
        spark.sparkContext.parallelize(jsons, numSlices=840), multiLine=True
    ).dropDuplicates()
...
def df_to_jdbc(a_df, adtfeed):
    ...
    try:
        ...
        if "not found in schema" in str(e_error):
            _, psql_cursor = psql_connection(config)
...
def create_table(table, df_columns, psql_cur):
    ...
    for db_col in df_columns:
        add_column_query = (
            f"ALTER TABLE {table} ADD COLUMN IF NOT EXISTS {db_col} text;"
        )
...
if __name__ == "__main__":
    # Setup logging (already imported in utils, but keeping for specific format if needed)
    logging.basicConfig(
    ...
    # Override some specific configs if needed
    spark.conf.set("spark.driver.memory", "14g")
    spark.conf.set("spark.debug.maxToStringFields", "200")
